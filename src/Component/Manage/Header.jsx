...
const ContentHeader = ({children}) => {
    ...
    const handleSelectChangeStatus = (event) => {
        setSelectValueStatus(event.target.value);
    };
    ...
    <>
          ...
          <Box sx={{ position: 'relative', zIndex: 1, width: '100%' }}>
            <Grid container alignItems="center" justifyContent="space-between" spacing={2}>
                <Grid item xs={12} md={6}>
                    <Typography variant="h2" sx={{ paddingLeft: 2, fontWeight: 'bold' }} >MANAGE USERS</Typography>
                </Grid>
                <Grid item xs={12} md={6}>
                  <Grid container spacing={2} justifyContent="flex-end">
                      ...
                      <Grid item xs={6} md={4}>
                        <FormControl sx={{width: 180}}>
                          <Select value={selectValueRole} onChange={handleSelectChangeRole} displayEmpty fullWidth input={<CustomSelect />}>
                              <MenuItem value="" sx={{ display: 'none' }}>Select User Role</MenuItem>
                              {Object.keys(userRoles).map((role, index) => {
                                    return <MenuItem value={userRoles[role]} key={index}>{role.replace('_', ' ')}</MenuItem>
                                })
                              }
                          </Select>
                        </FormControl>
                      ...
                      <Grid item xs={6} md={4}>
                        <FormControl sx={{width: 180}}>
                          <Select value={selectValueStatus} onChange={handleSelectChangeStatus} displayEmpty fullWidth input={<CustomSelect />}>
                              <MenuItem value="" sx={{ display: 'none' }}>Select Status</MenuItem>
                              {['deactivate', 'activate'].map((status, index) => {
      ...
      {React.Children.map(children, (child) => {
        if (child.type === UserList) {
          return React.cloneElement(child, { society: dispalySocietyList, selectedSociety: selectValueSociety, selectedRole: selectValueRole, selectedStatus: selectValueStatus});
        }
        return child;
      })}
    </>
